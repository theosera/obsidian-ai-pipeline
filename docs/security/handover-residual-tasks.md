...
- ⚠️ **ドリフトは過去の話ではなく、起票と同日に再発している**: 2026-09-03 22:50 に py 版へ
  修正 (`844e4b4` / hook 16 行 + テスト 119 行 / 90 passed・8 xfailed) が**cjs 版の同時更新なしで
  着地**した (62 実測。所有者確認待ちで未 push)。⇒ **2 実装の同時更新を守る仕組みが無い**
  (規約はあるが、破っても何も鳴らない)。手順 (3) のテストによる pin が要点。
