...
| #18 | egress hook 2 実装間の設計 drift (cjs 版が旧設計のまま) | obsidian / SDK / youtube | 4–8h | 中 | 運用/回帰 |
...
## #18 — egress hook 2 実装間の設計 drift (#16 の 4 系統レビューの前提事実)

> ⚠️ **スコープの限定** (レビュー指摘により訂正): 本項が示すのは **egress hook の cjs 版と py 版という
> 2 実装の食い違い**である。CLAUDE.md の SLA はこの 2 実装を**まとめて 1 系統**として数えるため、
> ⛔ **これは「4 系統の同時更新が破れている」証拠ではない。** 残る 3 系統 (gitleaks rev /
> ops-logging の `mask()` / vault-ops の `SECRET_ERE`) が古いかどうかは**未測定**。

- **対象リポ**: `obsidian-ai-pipeline` (cjs 版) / `pipeline-youtube-SDK` (py 版の実体) /
  `pipeline-youtube` (py 版の写し)
...
- ⚠️ **ドリフトは過去の話ではなく、起票と同日に再発している**: 2026-09-03 に py 版へ修正
  (hook 16 行 + テスト 119 行 / 90 passed・8 xfailed) が**cjs 版の同時更新なしで着地**した
  (62 実測)。⇒ **2 実装の同時更新を守る仕組みが無い** (規約はあるが、破っても何も鳴らない)。
  手順 (3) のテストによる pin が要点。
  - 該当 commit は **`20713dd`** (as-of 2026-09-04 / 62 実測)。当初 `844e4b4` として push されたが
    `ruff format --check` で CI が落ち、整形のみの修正 (1 insertion / 3 deletions) で `20713dd` に
    なった。⭕ **hook 本体とテスト結果 (90 passed / 8 xfailed) は不変**。
  - SDK 側 PR (`theosera/pipeline-youtube-SDK#133`) は必須 5 件すべて pass で、残る merge blocker は
    **未解決レビュースレッド 1 本のみ**。⇒ push 判断は完了している。
- ⛔ **同期時の前提に注意**: py 版は **span 方式へは移行しておらず、置換方式のまま**である
  (62 実測)。「py 版が span 化された」という前提で cjs 版を合わせないこと。
- **リスク**: **1 系統の内部で 2 実装が食い違ったまま運用されている**。「層が無い」のではなく
  「**層が古い**」。cjs 版の B' は動詞を列挙しない経路 (インタプリタ経由の読み出し等) を通す。
  ⇒ #16 の四半期レビューは「4 系統を横に突き合わせる」規約だが、**系統 1 の内部が既に割れている**
  ため、まず**この 2 実装をどちらに寄せるか**を決めないと横の突き合わせが始められない。
- **手順**:
...
- **リスク**: lookahead を調整する限り、綴りを変えるたびに同族が再発する。**2026-09-03 に 2 回再発**:
  ...
  ⇒ ⭕ **本リポの cjs 版にこの置換方式は無い** (line 59 の免除は全文に対する述語で `replace` を
  伴わない / 2e 実測)。⇒ cjs 版が持つのは #18 の**ゼロ幅先読み族のみ**で、**置換族は py 版固有**。
  同族ではあるが**同一ではない**ので、修正を機械的に横展開しないこと。
- ⚠️ **写し (`pipeline-youtube`) は未修正のまま**: 当該ファイル (11,179 B / sha `bb262ff42bf6c557`)
  が **SDK の修正前の親と byte 一致**しており (差分 +15 −1 が SDK の修正 commit の numstat と一致)、
  ⇒ **上の 1.・2. の回避路 2 件が開いたまま**である。実体 (SDK) を直しても写しは直らない。
  - ⛔ **限定**: これは**内容の同一性からの推定**であって、`pipeline-youtube` 側でスイートを
    再実行して得た結果ではない (62 実測 / as-of 2026-09-04 / **2e 未検証**)。着手時は現地で
    再測すること。
...
- **現状**: 秘密ファイルの Read に対する deny がリポごとに散在している (測定値は
  ...
  と書いているが、**その雛形サフィックス集合に対応する相方の deny が、SDK にも connector にも
  存在しない** (0a 実測 / connector は 62 実測)。⇒ コメントが存在しない相方を参照している。
  ⚠️ ⛔ **「connector に deny が無い」という意味ではない** — connector は Read 18 件を含む deny を
  持つ (被覆表 §2)。⭐ 欠けているのは**雛形サフィックス定義と対になる特定の集合**であって、
  deny 全般ではない。⛔ その集合を構成するパターンの列挙は、py 版 hook を読む必要があり**未測定**。
