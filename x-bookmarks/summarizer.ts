...
/**
 * untrusted 本文の無害化は `./untrusted_text` が正典 (純関数のみ / 外部 SDK 非依存)。
 *
 * `hands_on_generator.ts` も同じ関数を使うが、そちらが本ファイル経由で import すると
 * `classifier.ts` の Anthropic / OpenAI クライアント生成まで巻き込むため、依存の無い
 * モジュールへ切り出した。**既存の import 元 (テスト含む) を壊さないよう re-export する。**
 */
export {
  sanitizeForLLM,
  escapeBatchItemBoundary,
  escapeUntrustedFence,
  neutralizeUntrusted,
} from './untrusted_text';
import { sanitizeForLLM, escapeBatchItemBoundary } from './untrusted_text';
